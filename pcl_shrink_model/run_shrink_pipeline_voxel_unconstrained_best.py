...
from pcl_deg_model_voxel_unconstrained_best import (
    CrystallinityParams,
    TwoPhaseParams,
...
class RunParams:
    ...
    accel_factor: float = 1.03
    k_erosion: float = 0.12
    transition_width: float = 1.60
    phase2_max_fraction: float = 0.12
    erosion_onset_ratio: float = 0.20
    erosion_power: float = 2.0
    min_mass_fraction: float = 0.0

    kd_surface_boost: float = 1.12
    kd_core_drop: float = 0.94
    curvature_boost: float = 0.0#0.08
    ...
    erosion_start_mn_ratio: float = 0.20
    erosion_full_mn_ratio: float = 0.05
    max_voxel_remove_fraction_per_step: float = 0.0015
...
def local_rate_multiplier(mask: np.ndarray, p: RunParams) -> tuple[np.ndarray, np.ndarray]:
    ...
    mult = np.clip(mult, 0.92, 1.15).astype(np.float32)
...
def evolve_mask_continuous(current_mask, mn_ratio, risk, local_mult, mn_remove_thresh=0.20):
    ...
    p_remove *= (0.25 + 0.75 * surface.astype(np.float32))
    p_remove *= local_mult.astype(np.float32)
    p_remove = np.clip(p_remove, 0.0, 0.25)

    rand = np.random.rand(*current_mask.shape)
...
def main() -> None:
    ...
    with open(summary_path, "w", encoding="utf-8") as csv:
        ...
        for i, t in enumerate(times):
            local_mult, curv_grid = local_rate_multiplier(current_mask, p)
            grid_fields = grid_fields_at_t(float(t), local_mult, curv_grid, p, k0, sav0, two_phase, cryst_params)
            t_grid = np.linspace(0.0, max(float(t), 1e-9), 240)
            global_mass_frac_target = float(mass_fraction_two_phase(t_grid, p.mn0, k0, sav0, p.m, two_phase)[-1])
            ...
            #         float(t), local_mult, curv_grid, p, k0, sav0, two_phase, cryst_params
            #     )
            ...
            face_fields = {key: sample_faces_from_grid(face_centers, origin, pitch, np.asarray(arr, dtype=np.float32)).astype(np.float32)
            ...
            print(f"step {i:03d}  t={t:.3f} yr  Mn/Mn0 mean={np.mean(mr_live):.3f} [{np.min(mr_live):.3f}-{np.max(mr_live):.3f}]  target_mf={global_mass_frac_target:.3f} real_mf={real_mass_frac:.3f}  voxels={np.count_nonzero(current_mask)} comps={ncomp}")
            csv.write(f"{i},{t:.6f},{np.mean(mr_live):.6f},{np.min(mr_live):.6f},{np.max(mr_live):.6f},{np.mean(e_live):.6f},{global_mass_frac_target:.6f},{real_mass_frac:.6f},{np.mean(phi_live):.6f},{np.mean(risk_live):.6f},{np.count_nonzero(current_mask)},{ncomp}\n")
